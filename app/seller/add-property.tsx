...
import { X, Camera, Plus, Home, DollarSign, Bed, Bath, Square, Tag } from 'lucide-react-native';
...
export default function AddPropertyScreen() {
  ...
  const propertyTypes: PropertyType[] = ['house', 'apartment', 'Hotels & Lodges', 'townhouse', 'Commercial Warehouse'];
  ...
  const pickImage = async () => {
    try {
      ...
      if (isDemoMode) {
        // In demo mode, add a placeholder image if image picker fails
        setImages([...images, 'https://images.unsplash.com/photo-1564013799919-ab600027ffc6']);
      }
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (!title || !description || !address || !city || !state || !zipCode || !price || !beds || !baths || !sqft) {
    ...
    try {
      const newProperty = {
        ...
        has3DTour,
        tourUrl: has3DTour ? tourUrl : undefined,
      ...
      {isDemoMode && (
        <View style={styles.demoModeContainer}>
          <Text style={styles.demoModeText}>
            Demo Mode: Property creation is simulated
          </Text>
        </View>
      )}
